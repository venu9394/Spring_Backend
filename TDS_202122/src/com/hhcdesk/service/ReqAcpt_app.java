 ...
 */
public class ReqAcpt_app extends HttpServlet {
   ...
   if(Authcode>0){
	   ...
	   //****************************************************************************
	 		...
	 		if(Test_Flag!=null && Test_Flag.equalsIgnoreCase("P") && test_EMPLOYEEID.equalsIgnoreCase(""+Emp_id+"")){
	 			
	 			Atten_Req_Message="Self Approval Not allowed..!";
	 			auth_apr_flag=false;
	 ...
	 if(auth_apr_flag){
		try{
			...
			System.out.println(empid +"~ "+flagupdate+"~"+rid+"~"+comments);
			//14~ 84~Approve~null
			//comments="APPROVED FROM APPLICATION";
			comments="NA";
			...
			updateflag.append("UPDATE hclhrm_prod_others.tbl_emp_attn_req set FLAG='"+flagupdate+"',comments= '"+comments+"',approvedby='"+Emp_id+"',approvedname='"+Emp_name+"' , MAIL_STATUS='N' WHERE FLAG in('P','A') AND EMPLOYEEID="+empid+" AND RID="+rid+" ");

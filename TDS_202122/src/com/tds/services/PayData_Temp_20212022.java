...
import com.hhcdesk.service.login;

@SuppressWarnings({ "serial", "unused" })

public class PayData_Temp_20212022 extends HttpServlet {
	...
	*/
	...
	static final Logger LOGGER = Logger.getLogger(PayData_Temp_20212022.class);
	...
	//String FromPeriod="201609";
	   String FromPeriod="202104";
	   
	   String ToPeriod="202203";
	   
	  // String ToPeriod="201910";
	   
	   int currentPayperiod=202105;
	   
	   int ClosedPayperiod=202104; 
	   
	   // need to change current payperiod every time every month
	   String TaxStartMonth="2021-04-01";
	   String TaxEndMonth="2022-03-31";
	...
	//  int Rem_Months=1;
	  ...
	  /* 30055  PD & CTC annual bonus issue...!*/
     ...
     //   list_BU_la.add(29);
       ...
       list_BU_la.add(36);
        ...
        //10,11,14,15,16,17,18,19,20,21,22,24,26,27,28,29,33,34,36
    ...
    /*  list_BU_la.add(3);
     ...
     // String GetEMPID="20314";
      ...
      if(companyid!=null && companyid.equalsIgnoreCase("EMPID")){
			...
			Employee_ID_Seq_Bulk.append(" a.employeeid=b.employeeid   ");
			//Employee_ID_Seq_Bulk.append(" and b.businessunitid=a.companyid ");
			Employee_ID_Seq_Bulk.append(" where  a.employeesequenceno in (10426 ,10556 ,10590 ,10608 ,10634 ,10766 ,10781 ,11140 ,11505 ,30123) and  ");
			Employee_ID_Seq_Bulk.append(" b.payperiod between "+FromPeriod+" and "+ToPeriod+" ");
			Employee_ID_Seq_Bulk.append(" group by a.employeeid limit 1");
			*/
...
if(allowWhile && empflag) {
      ...
      while(Bu_List_lat.hasNext()){   /// Main Bu Iterator opining For Bu Iteratero
    	  ...
    	  FromPeriod="202104";
    	  ToPeriod="202203";
    	  currentPayperiod=202105; 
    	  ClosedPayperiod=202104;  
	...
	/*if(EmpParam!=null){
		...
		Eq_emp_plan="EMPID";
		   ...
		   Employee_ID_Seq_Bulk.append(" select employeeid,employeesequenceno,companyid from hclhrm_prod.tbl_employee_primary where employeesequenceno in("+Emp_Seq_Number+") and status in(1001,1061) ");
		    	//** changed for remove employess list Data*/
		  // Employee_ID_Seq_Bulk.append(" select employeeid,employeesequenceno,companyid from hclhrm_prod.tbl_employee_primary where employeesequenceno in("+Emp_Seq_Number+")  and status in(1001,1092,1401,1061,1091,1161,1601) ");
		    
		    
		   // Employee_ID_Seq_Bulk.append(" select employeeid,employeesequenceno,companyid from hclhrm_prod.tbl_employee_primary where employeesequenceno in (10426 ,10556 ,10590 ,10608 ,10634 ,10766 ,10781 ,11140 ,11505 ,30123) ");
...
//							currentPayperiod=Integer.parseInt(rs.getString("prdcurrent")); //enable after one payroll
		    ...
		    /*	Employee_ID_Seq_Bulk.append(" select  distinct a.employeeid,a.employeesequenceno   from    ");
		    	Employee_ID_Seq_Bulk.append(" hclhrm_prod.tbl_employee_payperiod_details b ");
		    	Employee_ID_Seq_Bulk.append(" left join hclhrm_prod.tbl_employee_primary a on ");
		    	Employee_ID_Seq_Bulk.append(" a.employeeid=b.employeeid   ");
		    ...
		    //	Employee_ID_Seq_Bulk.append(" select employeeid,employeesequenceno from hclhrm_prod.tbl_employee_primary where companyid in("+companyid+")  and status in(1001,1092,1401)");
		    	
		    	 //## to run outer chart
		    	
		   	Employee_ID_Seq_Bulk.append(" select employeeid,employeesequenceno from hclhrm_prod.tbl_employee_primary where companyid in("+companyid+") and status in(1001,1061) ");
			    
		    	//Employee_ID_Seq_Bulk.append(" select employeeid,employeesequenceno from hclhrm_prod.tbl_employee_primary where companyid in("+companyid+") and status in(1001,1092,1401,1061,1091) ");
	...
	//	yeardata.append(" select b.employeesequenceno,a.payperiod,a.payabledays,a.grossvalue,a.netvalue,if(a.componentid=22,'Gross',if(a.componentid=56,'LTA',if(a.componentid=87,'Arrear',if(a.componentid=71,'PB', if(a.componentid=80,'INCENTIVE',if(a.componentid=46,'Paid Net(include others)', if(a.componentid=69,'Annual Bonus',if(a.componentid=94,'Bonus',if(a.componentid=57,'PB',''))))  ))))),b.callname ");
		...
		yeardata.append(" where a.componentid in (22,87,80,56,71,69,94,57,97,98,99,100,88,60) and "); // ADDITIONS ADDING
		...
		ArrearCom.append(" where a.componentid in (87,80,56,71,79,90,69,94,57,99,100,88,60) and ");  // 88 ADD FOR ADDITIONS on 28-12-2017
	...
	//	EmpSequence.append(" select employeesequenceno,callname from hclhrm_prod.tbl_employee_primary where employeeid in ("+EmployeeIDs_Bulk+") and companyid="+companyid+" ");
      ...
      //  System.out.println("6::"+buunit.toString());
		...
		*/
		...
		Tax_dec.append(" select max(refno) from TEST.TBL_EMP_IT_TDS where employeeid in("+Employee_Seq_Bulk+") and status in(1001) and fy='"+FiyYear+"' group by employeeid ) ");
		Tax_dec.append(" and for_relation='Mother&Father' ");
		Tax_dec.append(" )BX ON BX.refno=g.refno ");
		Tax_dec.append(" WHERE ");
		Tax_dec.append(" g.refno in(select max(refno) from TEST.TBL_EMP_IT_TDS where employeeid in("+Employee_Seq_Bulk+") and status in(1001) and fy='"+FiyYear+"' group by employeeid) ");
			...
			//System.out.println(rs.getString(1));
			   ...
			   //YEAR CODES	
				...
				// For Annual Caliculation	
					...
					Basic.put(rs.getString(1)+"._60_EL", 0);
		...
		try {
			...
			if(rs.getString(2).equalsIgnoreCase("80")){ //Insentive
				...
				Basic.put(rs.getString(1)+"._57_ANN", Double.parseDouble(rs.getString(4).toString()));
  			   ...
  			   else if(rs.getString(2).equalsIgnoreCase("60") ){ // Referal OTHER LTA added for PB on 15-09-2017
				  
  				Basic.put(rs.getString(1)+"._60_EL", Double.parseDouble(rs.getString(4).toString()));
			...
			Basic.put(rs.getString(1)+".ANN_EAR_OTHERS", Double.parseDouble(Basic.get(rs.getString(1)+"._80_ANN").toString())
					...
					+Double.parseDouble(Basic.get(rs.getString(1)+"._71_ANN").toString())+Double.parseDouble(Basic.get(rs.getString(1)+"._94_ANN").toString())+Double.parseDouble(Basic.get(rs.getString(1)+"._69_ANN").toString())+
					 ...
					 Double.parseDouble(Basic.get(rs.getString(1)+"._60_EL").toString())
					  );
...
/*	87 /arrear
	...
	//	System.out.println("components_fetch.toString().trim()::"+components_fetch.toString().trim());
		...
		try {
				...
				try{
					...
					Basic.put(rs.getString(3) +"-"+rs.getString(4) , rs.getString(2));
					 ...
					 if(rs.getString(4).equalsIgnoreCase("39")){
					   ...
					   if(rs.getString(3).equalsIgnoreCase("3005511111111")){
						   
							 Basic.put(rs.getString(3)+"_ANNUAL_BENIFITS",Double.parseDouble(Basic.get(rs.getString(3) +".ANN_LTA").toString())+Double.parseDouble(Basic.get(rs.getString(3) +".ANN_BONUS").toString()));
			...
			// VENU Annual Benefits  make as 0
					
				// Basic.put(rs.getString(3)+"_ANNUAL_BENIFITS",0); 
					 
					 
					 
					 // for who have paid months and paid gross same with respect store
	...
	TDS_DEC_NEW_MOD.append(" select max(RefNo) RefNo,employeeid from test.tbl_emp_it_tds where status in(1001) and fy='"+FiyYear+"' ");
	...
	TDS_DEC_NEW_MOD.append(" where A.fy='"+FiyYear+"' and A.EMPLOYEEID IN("+Employee_Seq_Bulk+") and A.status in(1001) ");
...
//SELECT  EMPLOYEEID, FY, HQ, AGE, KIDCOUNT, SEC80C,NPS,S80D,D80D,H80D,S80GP,S80GAMT,RENT,RENTINTEREST,RENTLETOUT,RENT80EE,OTHERINCOME,LTA,STATUS, LUPDATE FROM test.tbl_emp_it_tds;			
		...
		//	rs=(ResultSet)DataObj.getBasic("SELECT  EMPLOYEEID, FY, HQ, AGE, KIDCOUNT, SEC80C, SEC80OTHERS, SEC80D, SEC13AHOUSE, HOUSELOAN, NPS,STATUS, LUPDATE FROM test.tbl_emp_it_tds where fy='2016-2017'", "ProposedPeriod", rs ,connection);
			...
			//	 NofMonths=Integer.parseInt(Basic.get(rs.getString(1)+".pm").toString())+Integer.parseInt(ProposedMonths);
				...
				//LetOutProperty=(((RentIntrest_LetOut*NofMonths)-MTP_LetOut)-((((RentIntrest_LetOut*NofMonths)-MTP_LetOut))*0.3));
				 ...
				 double  LetOutProperty_temp=0;
				 double  LetOutProperty_temp1=0;
				 
			       if(RentIntrest_LetOut>=MTP_LetOut){
			    	   
			    	   if(RentIntrest_LetOut>0){
			    	//   LetOutProperty=(((RentIntrest_LetOut*NofMonths)-MTP_LetOut)-((((RentIntrest_LetOut*NofMonths)-MTP_LetOut))*0.3));
			    	   LetOutProperty_temp1=RentIntrest_LetOut * NofMonths ;
			    	  // double LetOutProperty_s=(((RentIntrest_LetOut*NofMonths)-MTP_LetOut))*0.3);
			    	   
			    	   if(LetOutProperty_temp1<MTP_LetOut){
			    		   MTP_LetOut=0.00;
			    	   }
			    	   
			    	   LetOutProperty_temp=LetOutProperty_temp1 - MTP_LetOut ;
			    	   
			    	   LetOutProperty_t=LetOutProperty_temp * 0.3 ;
			    	   
			    	   LetOutProperty=LetOutProperty_temp - LetOutProperty_t;
			    	   }/*else{
			    		   
			    		   LetOutProperty=0-MTP_LetOut;
			    	   }*/
		...
		while(empseq.hasNext()){    // Caliculation Part Start;
			...
			try{
					...
					// ADD by VENU 23-02-2021
					double Arr_basic=Double.parseDouble(Basic.get(empid+"._87_ANN").toString());
					double Arr_basic_OF_BASIC=Arr_basic*0.5 ;
					double Arr_basic_40_p=Arr_basic_OF_BASIC*0.4;
					double Arr_hra=Arr_basic_OF_BASIC * 0.5;
					double Arr_basi_10=Arr_basic_OF_BASIC * 0.1;
				//// VENU ************************************************************************************************
			...
			//*****************************************************************************************
					// VENU-CHANGE2
				System.out.println(empid+"~ProposedMonths --->:" + ProposedMonths);
				
				//|| Integer.parseInt(ProposedMonths)==1
		...
		//	FinalComponents.put(empid+"-gross", formatter.format((Newgross+Oldgross)+Double.parseDouble(Basic.get(empid+".LTA").toString())));
			...
			//VENU
			//FinalComponents.put(empid+"-gross", "0");
		...
		// ** VENU12 ProposedMonths="12";
			...
			LOGGER.debug(empid+"~"+totalperiod_pm+"~"+HouseRent_pm+"~"+HouseRent+"~"+totalperiod);
			//com.tds.services.PayData_Temp_201920  - 10419~10~1400000.0~1680000.012
			//************ NPS Caliculation ***************************
			...
			LOGGER.debug(empid+"~"+HouseFinal+"~"+HouseRent_pm+"~"+HouseRent+"~"+HouseFinal_y);
			//com.tds.services.PayData_Temp_201920  - 10419~10~1400000.0~1680000.012
			...
			LOGGER.debug(empid+"~"+HouseRent_pm+"~"+HouseFinal+"~"+(OldHra_y+NewHra)+"~"+Basi40+"~"+(HouseRent_pm-HouseFinal));
			//com.tds.services.PayData_Temp_201920  - 10419~10~1400000.0~1680000.012
			...
			if((HouseRent_pm-HouseFinal)>0){
				...
				LOGGER.debug(empid+"~i am in loop~"+HouseRentTotal);
			...
			if((HouseRent-HouseFinal_y)>0){
				...
				//Changed By 15-12-2016
				//HouseRentTotal=Math.min(Basi40, (HouseRent-HouseFinal),(OldHra+NewHra));
		...
		//	System.out.println("Test Case 6");
			...
			if(_RENT80EE_Temp>150000){
				_RENT80EE=(double)150000;
				
			}else{
				_RENT80EE= _RENT80EE_Temp;
			}
			
		    TaxSections.put(empid+"_RENTINTEREST_MAIN", _RENTINTEREST_Temp+_RENT80EE);
	...
	//****************************************************** VENU25 (25-08-2020)**********************************	
			...
			if(_RENTINTEREST_Temp>200000){
	          ...
	          double $FINAL_RENTINTEREST=_RENT80EE+_RENTINTEREST;
	
	          TaxSections.put(empid+"_RENTINTEREST_E", _RENTINTEREST+_RENT80EE);
	          
	         _RENTINTEREST= _RENTINTEREST_Temp; // ADDED FOR LATEST CHANGES AUG 25
	         TaxSections.put(empid+"_RENTINTEREST_EE", _RENTINTEREST+_RENT80EE);
	         
	//****************************************************** VENU25 (25-08-2020)**********************************			
			//Below Displayed for RentIntrest changed by venu 20-12-2016
		//	TaxSections.put(empid+"_RENTINTEREST_E", _RENTINTEREST); // ADDED FOR LATEST CHANGES AUG 25
		...
		//	double MedicalTotal_y=Math.min((OldMedical_y+NewMedical), 1250 * totalperiod);
		  ...
		  //	ChildrenEdu=Math.min((OldEducation+NewEducation), 200 * totalperiod);
			...
			//****DATA 25-01-2019  _CA_M Removed
			
			
		//	ExmpAmount=HouseRentTotal+CaTotal+MedicalTotal+ChildrenEdu_pm+Double.parseDouble(TaxSections.get(empid+"_LTA_P").toString());
			
			ExmpAmount=HouseRentTotal+MedicalTotal+ChildrenEdu_pm+Double.parseDouble(TaxSections.get(empid+"_LTA_P").toString());
		...
		//	ExmpAmount_y=HouseRentTotal_y+CaTotal_y+MedicalTotal_y+ChildrenEdu+Double.parseDouble(TaxSections.get(empid+"_LTA_P").toString());
			...
			//****DATA 25-01-2019  CaTotal_y  Removed 
			ExmpAmount_y=HouseRentTotal_y+MedicalTotal_y+ChildrenEdu+Double.parseDouble(TaxSections.get(empid+"_LTA_P").toString());
	...
	//***********************************************************************************Final Income **********************************
		//	FinalIncome=( (Newgross+Oldgross+Double.parseDouble(Basic.get(empid+".LTA").toString()))-(OldPt+NewPt))-ExmpAmount;
			...
			//ExmpAmount=ExmpAmount+CaTotal;
					
		//	FinalIncome=( (Oldgross+Double.parseDouble(Basic.get(empid+".LTA").toString()))-(OldPt))-ExmpAmount;
			
		//	FinalIncome=( (Oldgross+Double.parseDouble(Basic.get(empid+".LTA").toString()))-(OldPt))-ExmpAmount;
			
			FinalIncome=( (Oldgross+Double.parseDouble(Basic.get(empid+".LTA").toString()))-(OldPt))-ExmpAmount-CaTotal;
			
			double FinalIncome_y=0.00;
			
			//ExmpAmount_y=ExmpAmount_y+CaTotal_y;
	...
	//	Double ANN_FinalIncome=( (Newgross+Oldgross_y+Double.parseDouble(Basic.get(empid+".ANN_LTA").toString())+Double.parseDouble(Basic.get(empid+".ANN_BONUS").toString()))-(OldPt+NewPt))-ExmpAmount;	
			...
			Double ANN_FinalIncome=( (Newgross+Oldgross_y+Double.parseDouble(Basic.get(empid+"_ANNUAL_BENIFITS").toString()))-(OldPt_y+NewPt))-ExmpAmount_y-CaTotal;	
	...
	//**********************************************************************************Final Income End *******************************		
		...
		//	 Double.parseDouble(Basic.get(empid+".MEDICAL_ANNUAL").toString()
			...
			/* VENU CHANGED on 03-12-2020  ADDING CTC_MEDICAL in OUTER LIST */
			 
		    double S80D=Double.parseDouble(TaxSections.get(empid+"D80D").toString()) +  Double.parseDouble(Basic.get(empid+".MEDICAL_ANNUAL").toString());
			...
			// Double.parseDouble(Basic.get(empid+".MEDICAL_ANNUAL").toString());
			 ...
			 double _$80DD_$$= Double.parseDouble(Basic.get(empid+".OUTER_D80D").toString())+Double.parseDouble(Basic.get(empid+".MEDICAL_ANNUAL").toString())
	...
	//System.out.println(FinalIncome+"~"+(Double.parseDouble(TaxSections.get(empid+"INHR").toString()))+"~"+(Double.parseDouble(TaxSections.get(empid+"_OTHERINCOME").toString()))+"~"+TotalS80Cal+"~"+S80D_total+"~"+_RENT80EE+"~"+_RENTINTEREST+"~"+empid);
		...
		//    ANN_FinalIncome=ANN_FinalIncome+(Double.parseDouble(TaxSections.get(empid+"INHR").toString())+(Double.parseDouble(TaxSections.get(empid+"_OTHERINCOME").toString())));
          ...
          double INHR_1718=Double.parseDouble(TaxSections.get(empid+"INHR").toString()); // Less of House Property Limit Set to 200000
			...
			double Mainsum=0.00,subSum=0.00,subSum1=0.00;
					
			
			//double RENT_80E1718=Double.parseDouble(TaxSections.get(empid+"_RENTINTEREST_MAIN").toString());
				double RENT_80E1718=_RENT80EE+_RENTINTEREST;
			
			
			//  double  RENT_80E1718=$FINAL_RENTINTEREST;
			
			subSum=INHR_1718 - RENT_80E1718;
		
		//only for LetOut Property
			if(RENT_80E1718==0 && (INHR_1718<0 || INHR_1718>0)){
			...
			if (RENT_80E1718>0){ //housing LOan
				...
				if(subSum>0){
					...
					if(subSum1>=$FINAL_RENTINTEREST){
						Mainsum=0-$FINAL_RENTINTEREST;
					}else{
						Mainsum=0-subSum1;
					}
	  ...
	  //HOusing LOan
			...
			TaxSections.put(empid+"_RENTINTEREST_E" , Mainsum);
		...
		//	System.out.println(Tax_Temp1+"Tax_Temp1"+empid);
			...
			if( retval>0 && Emp_Age<60 ){     
				...
				//	System.out.println(Tax_Temp1+"Tax_Temp2"+empid);
					...
					double _807A=(Tax_Temp1 * 0.05);
					
						//Tax_Temp=(Tax_Temp1 * 0.05)-2500.00;
					
					Tax_Temp=(Tax_Temp1 * 0.05)-_807A; // FinalCial Year 2019-2020
				...
				/*	TaxSections.put(empid+"_EmpTax_E", "250000");
					...
					TaxSections.put(empid+"_EmpTaxAddl_E", ""+_807A+"");// FinalCial Year 2017-2018
					...
					TaxSections.put(empid+"_EmpTaxAddl_E", "0");
						...
						//***** Marginal Relieaf
						if(surcharge<=Relif && Txbleamt>=5000000){
							Relif=0.0;
						}else if(surcharge>=Relif && Txbleamt>=5000000 ){
							Double limit_Taxable_surcharge_1cr=((5000000-1000000)*0.3)+112500 ;
							Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.0;
							Double Difference=(Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
							Double Temp_Releafe=0.0;
							 if(Difference>Relif){
							      Temp_Releafe=Difference-Relif;
							 }else{
								 Temp_Releafe=Relif;
							 }
							surcharge=surcharge-Temp_Releafe;
						}else{
							surcharge=surcharge;
						}
				// ************* Marginal Relieaf END
				...
				}else if(Txbleamt>10000000 && Txbleamt<=20000000){
				...
				//***** Marginal Relieaf
					if(surcharge<=Relif && Txbleamt>=10000000){
						Relif=0.0;
					}else if(surcharge>=Relif && Txbleamt>=10000000 ){
						Double limit_Taxable_surcharge_1cr=((10000000-1000000)*0.3)+112500 ;
						Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.1 ;
						Double Difference=(Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
						Double Temp_Releafe=0.0;
						 if(Difference>Relif){
						      Temp_Releafe=Difference-Relif;
			...
			// ************* Marginal Relieaf END
				...
				//	Double Relif_dedu=0.00;
				...
				}else if(Txbleamt>20000000 && Txbleamt<=50000000){
					
                    Tax_Temp=((Txbleamt-1000000)*0.3)+112500; // FinalCial Year 2017-2018
					
					TaxSections.put(empid+"_Before_Dedu_tax", Tax_Temp);
					
					Double surcharge=Tax_Temp*0.25;
					Double Relif=(Txbleamt-20000000);
					/*Double Relif_dedu=0.00;
					...
					//***** Marginal Relieaf
					if(surcharge<=Relif && Txbleamt>=20000000){
						Relif=0.0;
					}else if(surcharge>=Relif && Txbleamt>=20000000 ){
						Double limit_Taxable_surcharge_1cr=((20000000-1000000)*0.3)+112500 ;
						Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.25 ;
						Double Difference=(Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
						Double Temp_Releafe=0.0;
						 if(Difference>Relif){
						      Temp_Releafe=Difference-Relif;
			...
			// ************* Marginal Relieaf END
				...
				}else if(Txbleamt>50000000){
					
                   Tax_Temp=((Txbleamt-1000000)*0.3)+112500; // FinalCial Year 2017-2018
					
					TaxSections.put(empid+"_Before_Dedu_tax", Tax_Temp);
					
					Double surcharge=Tax_Temp*0.37;
					
					Double Relif=(Txbleamt-50000000);
					/*Double Relif_dedu=0.00;
...
*/				
					if(surcharge<=Relif && Txbleamt>=50000000){
						Relif=0.0;
					}else if(surcharge>=Relif && Txbleamt>=50000000 ){
						Double limit_Taxable_surcharge_1cr=((50000000-1000000)*0.3)+112500 ;
						Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.37 ;
						Double Difference=(Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
						Double Temp_Releafe=0.0;
						 if(Difference>Relif){
						      Temp_Releafe=Difference-Relif;
			...
			//	System.out.println(empid +"~F-Tax_Temp~" +Tax_Temp);        // AGE 60
				...
				if(Txbleamt>300000 && Txbleamt<=500000){
					...
					if(Txbleamt<=500000){
						...
						double _80A=(Txbleamt-300000)*0.05;
						
						Tax_Temp=((Txbleamt-300000)*0.05)-_80A; // FY YEAR 2017-2018
						...
						TaxSections.put(empid+"_EmpTaxAddl_E", ""+_80A+"");
					...
					if(Txbleamt<=5000000){
						...
						TaxSections.put(empid+"_EmpTaxAddl_E", "0");
							...
							if(surcharge<=Relif && Txbleamt>=5000000){
								...
								Double limit_Taxable_surcharge_1cr=((5000000-1000000)*0.3)+110000 ;
								Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.0 ;
								Double Difference=(Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
								Double Temp_Releafe=0.0;
								 if(Difference>Relif){
					...
					if(surcharge<=Relif && Txbleamt>=10000000){
						...
						Double limit_Taxable_surcharge_1cr=((10000000-1000000)*0.3)+110000 ;
						Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.1 ;
						Double Difference=(Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
						Double Temp_Releafe=0.0;
						 if(Difference>Relif){
			...
			// ************* Marginal Relieaf END
					...
					if(surcharge<=Relif && Txbleamt>=20000000){
						...
						Double limit_Taxable_surcharge_1cr=((20000000-1000000)*0.3)+110000 ;
						Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.25 ;
						Double Difference=(Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
						Double Temp_Releafe=0.0;
						 if(Difference>Relif){
			...
			// ************* Marginal Relieaf END
					...
					if(surcharge<=Relif && Txbleamt>=50000000){
						...
						Double limit_Taxable_surcharge_1cr=((50000000-1000000)*0.3)+110000 ;
						Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.37 ;
						Double Difference=(Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
						Double Temp_Releafe=0.0;
						 if(Difference>Relif){
...
////////////////////////////////////////////////////////////////BELOW CHANGES FOR ANNUAL DTAA ////////////////////////////////////////////////////////////////////////
			    ...
			    if( ANN_retval>0 && Emp_Age<60  ){
					...
					//	System.out.println(Tax_Temp1+"Tax_Temp2"+empid);
						...
						if(ANN_Txbleamt<500001){
							
						double _87A=(Tax_Temp1 * 0.05);
					
						 //ANN_Tax_Temp=(Tax_Temp1 * 0.05)-2500.00; old
						
						ANN_Tax_Temp=(Tax_Temp1 * 0.05)-_87A;
					...
					//	TaxSections.put(empid+"_ANN_EmpTaxPer_E", "0.1");
						...
						TaxSections.put(empid+"_ANN_EmpTaxAddl_E", ""+_87A+"");
						}else{
					...
					//	ANN_Txbleamt=ANN_Txbleamt+500000; // ADD For Surcharge Caliculation
						...
						TaxSections.put(empid+"_ANN_EmpTaxAddl_E", "0");
							...
							//***** Marginal Relieaf
							if(surcharge<=Relif && ANN_Txbleamt>=5000000){
								Relif=0.0;
								 Relif_dedu=Relif;
							}else if(surcharge>=Relif && ANN_Txbleamt>=5000000 ){
								Double limit_Taxable_surcharge_1cr=((5000000-1000000)*0.3)+112500 ;
								Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.0;
								Double Difference=(ANN_Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
								 Double Temp_Releafe=0.0;
								 if(Difference>Relif){
								      Temp_Releafe=Difference-Relif;
								      Relif_dedu=Temp_Releafe;
					...
					}else if(ANN_Txbleamt>10000000 && ANN_Txbleamt<=20000000){
						ANN_Tax_Temp=((ANN_Txbleamt-1000000)*0.3)+112500;
						TaxSections.put(empid+"_ANN_Before_Dedu_tax", ANN_Tax_Temp);
					...
					//***** Marginal Relieaf
						Double Temp_Releafe=0.0;
						if(surcharge<=Relif && ANN_Txbleamt>=10000000){
							Relif=0.0;
							 Relif_dedu=Relif;
						}else if(surcharge>=Relif && ANN_Txbleamt>=10000000 ){
							Double limit_Taxable_surcharge_1cr=((10000000-1000000)*0.3)+112500 ;
							Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.1;
							Double Difference=(ANN_Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
							 Temp_Releafe=0.0;
				...
				// ************* Marginal Relieaf END
					...
					}else if(ANN_Txbleamt>20000000 && ANN_Txbleamt<=50000000){
						ANN_Tax_Temp=((ANN_Txbleamt-1000000)*0.3)+112500;
						TaxSections.put(empid+"_ANN_Before_Dedu_tax", ANN_Tax_Temp);
						Double surcharge=ANN_Tax_Temp*0.25;
						Double Relif=(ANN_Txbleamt-20000000);
						Double Relif_dedu=0.00;
						
					//***** Marginal Relieaf
						Double Temp_Releafe=0.0;
						if(surcharge<=Relif && ANN_Txbleamt>=20000000){
							Relif=0.0;
							 Relif_dedu=Relif;
						}else if(surcharge>=Relif && ANN_Txbleamt>=20000000 ){
							Double limit_Taxable_surcharge_1cr=((20000000-1000000)*0.3)+112500 ;
							Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.25;
							Double Difference=(ANN_Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
							 Temp_Releafe=0.0;
				...
				// ************* Marginal Relieaf END
					...
					}else if(ANN_Txbleamt>50000000){
						ANN_Tax_Temp=((ANN_Txbleamt-1000000)*0.3)+112500;
						TaxSections.put(empid+"_ANN_Before_Dedu_tax", ANN_Tax_Temp);
						Double surcharge=ANN_Tax_Temp*0.37;
						Double Relif=(ANN_Txbleamt-50000000);
						Double Relif_dedu=0.00;
						
					//***** Marginal Relieaf
						Double Temp_Releafe=0.0;
						if(surcharge<=Relif && ANN_Txbleamt>=50000000){
							Relif=0.0;
							 Relif_dedu=Relif;
						}else if(surcharge>=Relif && ANN_Txbleamt>=50000000 ){
							Double limit_Taxable_surcharge_1cr=((50000000-1000000)*0.3)+112500 ;
							Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.37;
							Double Difference=(ANN_Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
							 Temp_Releafe=0.0;
				...
				//	System.out.println(empid +"~F-Tax_Temp~" +Tax_Temp);
					...
					if(Txbleamt>=300000 && ANN_Txbleamt<=500000){
						...
						//if(ANN_Txbleamt<=350000){ old;
						
						if(ANN_Txbleamt<=500000){
						
							double _87A=((ANN_Txbleamt-300000)*0.05);
							
						//ANN_Tax_Temp=((ANN_Txbleamt-300000)*0.05)-2500;	
						ANN_Tax_Temp=((ANN_Txbleamt-300000)*0.05)-_87A;
						
						TaxSections.put(empid+"_ANN_Before_Dedu_tax", ((ANN_Txbleamt-300000)*0.05));
						TaxSections.put(empid+"_ANN_EmpTax_E", "300000");
						TaxSections.put(empid+"_ANN_EmpTaxPer_E", "0.1");
						TaxSections.put(empid+"_ANN_EmpTaxAddl_E", ""+_87A+""); //87A
				...
				// ************* Marginal Relieaf END
					...
					if(ANN_Txbleamt>500000 && ANN_Txbleamt<=1000000){
						...
						TaxSections.put(empid+"_ANN_EmpTaxAddl_E", "0");
							...
							if(surcharge<=Relif && ANN_Txbleamt>=5000000){
								...
								Double limit_Taxable_surcharge_1cr=((5000000-1000000)*0.3)+100000 ;
								Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.0;
								Double Difference=(ANN_Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
								 Temp_Releafe=0.0;
								 if(Difference>Relif){
					...
					// ************* Marginal Relieaf END
						...
						if(surcharge<=Relif && ANN_Txbleamt>=10000000){
							...
							Double limit_Taxable_surcharge_1cr=((10000000-1000000)*0.3)+100000 ;
							Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.1;
							Double Difference=(ANN_Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
							 Temp_Releafe=0.0;
							 if(Difference>Relif){
				...
				// ************* Marginal Relieaf END
						...
						if(surcharge<=Relif && ANN_Txbleamt>=20000000){
							...
							Double limit_Taxable_surcharge_1cr=((20000000-1000000)*0.3)+100000 ;
							Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.25;
							Double Difference=(ANN_Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
							 Temp_Releafe=0.0;
							 if(Difference>Relif){
				...
				// ************* Marginal Relieaf END
						...
						if(surcharge<=Relif && ANN_Txbleamt>=50000000){
							...
							Double limit_Taxable_surcharge_1cr=((50000000-1000000)*0.3)+100000 ;
							Double limit_Taxable_surcharge=limit_Taxable_surcharge_1cr * 0.37;
							Double Difference=(ANN_Tax_Temp+surcharge)-(limit_Taxable_surcharge_1cr+limit_Taxable_surcharge);
							 Temp_Releafe=0.0;
							 if(Difference>Relif){
